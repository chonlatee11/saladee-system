...
// Reusable catalog card (02-05). Renders one variety's photo/name, its server-
// resolved ฿ price for the customer (b2c) pack, a link into the variety detail, and
// an add-to-cart control. All money is DISPLAYED from server values (unitPriceSatang
// resolved by the api) and never computed on the client — the emitted `add` line
// carries ids + qty only (T-02-17). A depleted round shows the "หมดรอบนี้" sold-out
// badge (destructive) instead of the add control.
...
<template>
  <article class="rounded-xl bg-surface p-md">
    <RouterLink :to="`/variety/${variety.id}`" class="flex items-center gap-md">
      <img
