// Web-store client cart (04-09) — mirrors web/src/stores/cart.ts VERBATIM (one brand
// N apps). The guest checkout page (checkout.vue) consumes this shared store.
//
// SECURITY (T-04-29, mirrors T-02-17): the cart stores ONLY server-referencable ids +
// quantity — never a price/plants/money field. POST /orders re-resolves every price and
// the plant reservation server-side (the Phase-1 rule), so a tampered client cart can
// change WHAT is ordered but never WHAT it costs. The line shapes below are exactly the
// `OrderLineBody` / `BoxLineBody` the api accepts (api/src/routes/orders.ts).
//
// State is a module-level reactive singleton (no Pinia — keep the dependency count low,
// NFR-08) and is mirrored into sessionStorage so the multi-step checkout survives
// navigation between the catalog, the product page, and the checkout screen. sessionStorage
// access is guarded so the module imports cleanly under Nuxt SSR (no DOM/storage on server).
...
const STORAGE_KEY = "saladee.store.cart.v1";
...
/**
 * The cart API. A single shared store is returned to every caller (module singleton),
 * so the catalog view, the product page, and the checkout page mutate the same lines.
 */
